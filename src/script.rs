...
pub fn execute<T, B, V>(file: File, arguments: Vec<String>, app: &Application<T>)
    where T: ApplicationUI,
          B: BrowserWindow,
          V: WebView {
    let mut lua = create_runtime::<T, B, V>(app);
...
fn create_runtime<T, B, V>(app: &Application<T>) -> Lua
    where T: ApplicationUI,
          B: BrowserWindow,
          V: WebView {
    let mut lua = Lua::new();
    ...
    lua.set("resize_window", function3(|window_index: u8, width: u32, height: u32| {
        with_window::<T, B, _>(app, window_index, |w| w.resize(width, height));
    }));
    lua.set("close_webview", function2(|window_index: u8, webview_index: u8| {
        with_window::<T, B, _>(app, window_index, |w| w.close_webview(webview_index));
    }));
    lua.set("focus_webview", function2(|window_index: u8, webview_index: u8| {
        with_window::<T, B, _>(app, window_index, |w| w.focus_webview(webview_index));
    }));
    lua.set("load_uri", function3(|window_index: u8, webview_index: u8, uri: String| {
        with_window::<T, B, _>(app, window_index, |w| {
...
fn with_window<T, W, F>(app: &Application<T>, window_index: u8, callback: F)
    where T: ApplicationUI,
          W: BrowserWindow,
          F: FnOnce(&W) -> () {
    match app.ui.window(window_index) {
